...
for input in inputs:
    ...
    for i in range(len(points) - 1):
        ...
        if max(from_pt[0], to_pt[0]) > max_x:
            max_x = max(from_pt[0], to_pt[0])
        if min(from_pt[0], to_pt[0]) < min_x:
            min_x = min(from_pt[0], to_pt[0])
...
for i in range(min_x - max_y - 10, max_x + max_y + 10):
    walls.add((i, max_y + 2))
